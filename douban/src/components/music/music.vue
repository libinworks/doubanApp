<template>
	<div>
		<div id="search">
			<input type="search" v-on:blur="_search()" v-model="search" placeholder="请输入喜爱的歌手">
			<span @click="_search()">搜索</span>
		</div>
		<div v-if="result" id="moreMovie">
      <h1 v-if="result.total">共({{ result.total }})条</h1>
			<div id="movieLists">
	      <ul class="items">
	        <li v-for="item in result.musics">
	          <router-link :to="'/music/'+item.id">
	            <img class="movieImg" v-lazy="item.image" alt="">
...
export default {
  ...
  methods: {
    _search () {
      ...
      let url = 'https://api.douban.com/v2/music/search?q='
      this.$http.jsonp(url + starName, {params: {'start': 0, 'count': 100}}).then(function (data) {
